...
public class SrvThread extends Thread {
	...
	SrvThread(Socket csP, int idP, int modP) {
		...
		id = idP;
		/*
		 * Concurrent servers run in one of three modes: 0-ERROR 1-OK_ERROR 2-OK_OK
		 */
		mod = modP;
	}
	...
	private boolean opt0(String str_valor_comun, PrintWriter ac, BufferedReader dc)
			throws Exception {
	...
	private boolean opt1(String str_valor_comun, PrintWriter ac, BufferedReader dc, BigInteger bix,
		...
		if (linea.compareTo("ERROR") == 0) {
			...
			if (verificar) {
				...
				if (linea.compareTo("ERROR") == 0) {
					...
					System.out.println(
							"==========> Test 1c: passed (server sends not matching query and MAC).");
				} else if (linea.compareTo("OK") == 0) {
					System.out.println(
							"==========> Test 1c: failed (server sends not matching query and MAC).");
					exito = false;
				}
			} else {
				...
				ac.println(mensaje);
				System.out.println(
						"==========> Test 1b: failed (Client sends not matching query and MAC).");
				exito = false;
			}
		}
		return exito;
	...
	private boolean opt2(String str_valor_comun, PrintWriter ac, BufferedReader dc, BigInteger bix,
			PrivateKey privadaServidor) throws Exception {
		String linea;
		...
		if (linea.compareTo("ERROR") == 0) {
			...
			if (verificar) {
				...
				if (linea.compareTo("OK") == 0) {
					System.out.println(
							"==========> Test 2c: passed (server sends matching query and MAC).");
				} else if (linea.compareTo("ERROR") == 0) {
					System.out.println(
							"==========> Test 2c: failed (server sends matching query and MAC).");
					exito = false;
				}
				...
				ac.println(mensaje);
				System.out.println(
						"==========> Test 2b: failed (Client sends not matching query and MAC).");
				exito = false;
			}
		}
		return exito;
	...
	private BigInteger calcular_llave_maestra(BigInteger base, BigInteger exponente,
			BigInteger modulo) {
		return base.modPow(exponente, modulo);
	}
